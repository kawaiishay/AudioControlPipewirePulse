...
refreshcurvol() {
  [[ $PCV -eq 0 ]] &&
  CURVOL=$(pactl get-sink-volume @DEFAULT_SINK@ | awk '{ print $3 }' | sed -n -e 's,\([0-9]*\)%,\1,p') ||
    CURVOL=$(pactl get-sink-volume @DEFAULT_SINK@ | awk '{ print $5 }' | sed -n -e 's,\([0-9]*\)%,\1,p')
}

refreshsrcvol() {
  [[ $PCV -eq 0 ]] &&
  SRCVOL=$(pactl get-source-volume @DEFAULT_SOURCE@ | awk '{ print $3 }' | sed -n -e 's,\([0-9]*\)%,\1,p' ) ||
    SRCVOL=$(pactl get-source-volume @DEFAULT_SOURCE@ | awk '{ print $5 }' | sed -n -e 's,\([0-9]*\)%,\1,p'  )
  }
...
setup() {
  ...
  SOURCE_MUTED=$(pactl get-source-mute @DEFAULT_SOURCE@ | awk '/Mute:/{ print $2 }')
